...
{
    [CustomPropertyDrawer(typeof(DictionaryVariableReference))]
    public class DictionaryVariableReferenceDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
                ...
                }))
            {
                ...
                menu.AddItem(new GUIContent("Key"), referenceTypeIndex == 1, () => SetProperty(property, 1));
                menu.AddItem(new GUIContent("Value"), referenceTypeIndex == 2,() => SetProperty(property, 2));
                menu.ShowAsContext();
            }
            ...
            switch(referenceTypeIndex)
            {
                default:
                    ...
                    break;
                case 1:
                    EditorGUI.ObjectField(new Rect(position.x, position.y, position.width - 16, position.height), property.FindPropertyRelative("key"), typeof(ScriptableObject), GUIContent.none);
                    break;
                case 2:
                    EditorGUI.ObjectField(new Rect(position.x, position.y, position.width - 16, position.height), property.FindPropertyRelative("value"), typeof(ScriptableObject), GUIContent.none);
                    break;
            }
